...
## Image input / vision (local Ollama only)

**Date:** 2026-09-17 (pending commit)
**What:** The existing `POST /analyze-file` upload route now also accepts images (JPEG/PNG/WEBP/GIF): `server/adapters/ollama-vision.js` sends the uploaded bytes, base64-encoded, to a local multimodal Ollama model (`LOCAL_VISION_MODEL`, e.g. `llava`) via `/api/chat`'s `images` field, and the model's answer is logged directly as the chat response -- no separate caption-then-route step needed, since the vision model answers the question directly from the image.
**Why:** Continuing the "partial" items -- image input was the last one. First attempt used Hugging Face's free `hf-inference` tier (mirroring the existing image-generation/STT adapters), but a live check (this project's established practice before trusting any free-tier claim) found **zero** free hf-inference models for image-to-text/VQA/image-text-to-text as of 2026-09-17 -- the same class of "free tier evaporated" problem hit twice before with TTS and the decommissioned HF host. Rather than ship a feature that would fail on first use, stopped and asked the user; they chose local-Ollama-only (matching this project's existing local-first architecture for text), so image input only works when the operator has a multimodal Ollama model pulled and `LOCAL_VISION_MODEL` set -- otherwise it 503s with a clear message telling them what to do, rather than silently failing. Two subagent reviews ran before committing: the ecc security review found the local-only design has no SSRF surface (the request host only ever comes from `OLLAMA_BASE_URL`, never request data) and confirmed malformed/spoofed-mimetype uploads pose no risk since bytes are only ever base64-encoded into a local JSON API call, never parsed/executed/served back/written to disk. The caveman review's one real finding: the new integration test only checked that an `images` array of length 1 was sent, not that it contained the actual uploaded bytes -- strengthened to a `deepStrictEqual` against the real base64 encoding (a dedicated unit test in `test/ollama-vision.test.js` already had the stronger assertion; only the integration test was weak).
**Files:** `server/adapters/ollama-vision.js` (new), `server/files.js`, `client/src/ChatPage.jsx`, `.env.example`, `test/ollama-vision.test.js` (new), `test/files.test.js`.
**Tokens:** Subagent reviews only — caveman-reviewer: 86,887; ecc security-reviewer: 75,338. Main implementation cost isn't exposed by any available tool.

## Web-search grounding

**Date:** 2026-09-17 17:21
**What:** A "Web search" toggle chip; when on, `/chat` fetches live results from DuckDuckGo's free, no-key HTML endpoint (`server/adapters/web-search.js`, regex-parsed, redirect links decoded) and `server/web-grounding.js` folds them into the system prompt as explicitly-labeled untrusted reference data, capped at a 6s timeout so a slow search can't meaningfully delay the answer. Sources are returned alongside the response and rendered as a linked list under the answer, reusing the existing `isSafeHttpUrl` guard from the news feature.
**Why:** The last of the "biggest to smallest" no-credential items -- DuckDuckGo's HTML endpoint needs no API key or billing, consistent with every other integration in this project (Yahoo Finance, Google News, Hugging Face free tier). Two subagent reviews (caveman + ecc security) ran before committing, per standing instruction. The ecc review's one real finding: the fetched third-party title/snippet text was being concatenated raw into the LLM system prompt with no framing distinguishing it from instructions -- an SEO-poisoned search result could contain text like "ignore previous instructions" that the model might follow. Fixed by wrapping the results in explicit BEGIN/END markers and a preamble telling the model to treat them as untrusted data, never commands. The caveman review's one real finding: the grounding call had no timeout of its own, inheriting the adapter's full 15s internal ceiling and blocking `/chat` for that long in the worst case, unlike `suggestions.js`'s existing tighter timeout pattern -- fixed with the same `withTimeout` approach, capped at 6s. Both reviews confirmed no SSRF (the query is the only user-influenced part of a fixed-host URL, `encodeURIComponent`'d), no XSS-adjacent gap (resolved links are gated through `isSafeHttpUrl` before rendering, same as the existing news feature), and no ReDoS in the parsing regex (polynomial worst-case at most, not exponential).
**Files:** `server/adapters/web-search.js` (new), `server/web-grounding.js` (new), `server/chat.js`, `client/src/api.js`, `client/src/ChatPage.jsx`, `client/src/styles.css`, `test/web-search.test.js` (new), `test/web-grounding.test.js` (new), `test/chat.test.js`.
**Tokens:** Subagent reviews only — caveman-reviewer: 85,167; ecc security-reviewer: 81,499. Main implementation cost isn't exposed by any available tool.
